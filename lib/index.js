...
var _APIToolkit_topicName,
  _APIToolkit_topic,
  _APIToolkit_pubsub,
  _APIToolkit_project_id,
  _APIToolkit_config;
  ...
  constructor(pubsub, topicName, project_id, config) {
    ...
    if (__classPrivateFieldGet(this, _APIToolkit_pubsub, "f")) {
      __classPrivateFieldSet(
        this,
        _APIToolkit_topic,
        __classPrivateFieldGet(this, _APIToolkit_pubsub, "f").topic(
          __classPrivateFieldGet(this, _APIToolkit_topicName, "f"),
        ),
        "f",
      );
    }
    this.publishMessage = (payload) => {
      ...
      if (__classPrivateFieldGet(this, _APIToolkit_topic, "f")) {
        __classPrivateFieldGet(this, _APIToolkit_topic, "f").publishMessage(
          { json: payload },
          callback,
        );
      } else {
  ...
  static async NewClient(config) {
    ...
    if (clientMetadata == null || apiKey != "") {
      ...
      pubsubClient = new pubsub_1.PubSub({
        ...
        authClient: new pubsub_1.PubSub().auth.fromJSON(
  ...
  async expressMiddleware(req, res, next) {
    exports.asyncLocalStorage.run(new Map(), () => {
      ...
      exports.asyncLocalStorage
        ...
        .set(
          "AT_project_id",
          __classPrivateFieldGet(this, _APIToolkit_project_id, "f"),
        );
      exports.asyncLocalStorage
        .getStore()
        .set(
          "AT_config",
          __classPrivateFieldGet(this, _APIToolkit_config, "f"),
        );
      ...
      const onRespFinished = (topic, req, res) => (err) => {
              ...
              if (req.file) {
                ...
                if (!Array.isArray(req.files)) {
                  for (const file in req.files) {
                    req.body[file] = req.files[file].map(
                      (f) => `[${f.mimetype}_FILE]`,
                    );
                  }
                } else {
        ...
        const errors =
          exports.asyncLocalStorage.getStore()?.get("AT_errors") ?? [];
        const payload = (0, payload_1.buildPayload)(
          start_time,
          req,
          res,
          reqBody,
          ...
          __classPrivateFieldGet(this, _APIToolkit_config, "f")
            .redactResponseBody ?? [],
          __classPrivateFieldGet(this, _APIToolkit_config, "f").redactHeaders ??
            [],
          __classPrivateFieldGet(this, _APIToolkit_project_id, "f"),
          errors,
          __classPrivateFieldGet(this, _APIToolkit_config, "f").serviceVersion,
          __classPrivateFieldGet(this, _APIToolkit_config, "f").tags ?? [],
          msg_id,
          undefined,
        );
        if (__classPrivateFieldGet(this, _APIToolkit_config, "f").debug) {
      ...
      const onRespFinishedCB = onRespFinished(
        __classPrivateFieldGet(this, _APIToolkit_topic, "f"),
        req,
        res,
      );
...
(_APIToolkit_topicName = new WeakMap()),
  (_APIToolkit_topic = new WeakMap()),
  (_APIToolkit_pubsub = new WeakMap()),
  (_APIToolkit_project_id = new WeakMap()),
  (_APIToolkit_config = new WeakMap());
