    ...
    constructor(config, apiKey, projectId) {
        ...
        if (projectId) {
            ...
            if (!config.otelInstrumentated) {
                ...
                const undiciInst = new instrumentation_undici_1.UndiciInstrumentation({
                    requestHook: (span, request) => {
                    ...
                    responseHook: (span, { request, response }) => {
                        span.setAttribute("apitoolkit.parent_id", apitoolkit_js_1.asyncLocalStorage.getStore()?.get("apitoolkit-msg-id"));
                ...
                const sdk = new sdk_node_1.NodeSDK({
                    ...
                    resource: resource,
                    logRecordProcessors: [new sdk_node_1.logs.BatchLogRecordProcessor(logExporter)],
                    traceExporter,
                });
                this.otelSDk = sdk;
                sdk.start();
