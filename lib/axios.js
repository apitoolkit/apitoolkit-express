...
const apitoolkit_async_1 = require("./apitoolkit-async");
...
const onResponse = (urlWildcard, redactHeaderLists, redactRequestBody, redactResponseBody) => (response) => {
    if (apitoolkit_async_1.asyncLocalStorage.getStore() == null) {
        console.log("APIToolkitAsync: observeAxios used outside of the APIToolkitAsync middleware's scope. Use the APIToolkitAsyncClient.observeAxios instead, if you're not in a web context.");
        return response;
    }
    ...
    const project_id = apitoolkit_async_1.asyncLocalStorage.getStore().get("AT_project_id");
    const ATClient = apitoolkit_async_1.asyncLocalStorage.getStore().get("AT_client");
    const ATConfig = apitoolkit_async_1.asyncLocalStorage.getStore().get("AT_config");
    const parent_id = apitoolkit_async_1.asyncLocalStorage.getStore().get("AT_msg_id");
    var errors = [];
...
const onResponseError = (urlWildcard, redactHeaderLists, redactRequestBody, redactResponseBody) => (error) => {
    ...
    const respBody = JSON.stringify(res?.data || {});
    const project_id = apitoolkit_async_1.asyncLocalStorage.getStore().get("AT_project_id");
    const ATClient = apitoolkit_async_1.asyncLocalStorage
        .getStore()
        .get("AT_client");
    const ATConfig = apitoolkit_async_1.asyncLocalStorage.getStore().get("AT_config");
    const parent_id = apitoolkit_async_1.asyncLocalStorage.getStore().get("AT_msg_id");
